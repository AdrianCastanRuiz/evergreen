...
- `pnpm --filter @evergreen/api run test` — 6 suites, 66 tests, all passing (56 pre-existing + 10 new `inviteUser` unit tests + 4 new `sendHomeAccessAddedEmail` mail tests, minus overlap already counted).
...
**Modified:**
- `apps/api/src/users/users.service.ts` (added `inviteUser`, `grantExistingFamilyUserHomeAccess`, `ROLE_RANK`)
- `apps/api/src/users/users.service.spec.ts` (added `inviteUser` test block)
