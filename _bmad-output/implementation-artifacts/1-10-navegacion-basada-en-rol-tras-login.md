...
- [x] **Family `(tabs)` route group** — `apps/mobile/src/app/(tabs)/` with `index` (Home), `photos`, `events`, `menu`, `news` (AC: #1). Placeholder/empty-state screens; content screens belong to Epics 2/3/5/6. Root `index` (Home) hosta "My Profile" + "Log out" (FR4/FR9) — family no llega a `home.tsx` (hoy es la single-screen de staff), así que el tab shell es donde family cierra sesión (hallazgo de prueba manual).
...
- 2026-08-26: Prueba manual familiar — detectado que family, al enrutar a `(tabs)`, perdía el acceso a "Log out" (FR9) y "My Profile" (FR4), que antes vivían en `home.tsx` (hoy single-screen de staff). Añadidos ambos botones al tab Home `(tabs)/index.tsx`. ESLint + typecheck mobile en verde. Status → review.