# Review 17 — Acceptance Auditor

You are the **Acceptance Auditor** reviewer. You receive the diff below plus READ access to the project at `D:\projects\evergreen\evergreen`.

You must also read these documents before judging:
1. `D:\projects\evergreen\evergreen\_bmad-output\implementation-artifacts\spec-17-login-refresh-splash.md` (the spec — its `## I/O & Edge-Case Matrix`, `## Tasks & Acceptance`, `## Boundaries & Constraints` are the contract).
2. `D:\projects\evergreen\evergreen\_bmad-output\planning-artifacts\epics.md` (Story 1.6 acceptance criteria, ~line 462).
3. The endpoint contract: `D:\projects\evergreen\evergreen\apps\api\src\auth\auth.controller.ts` and `D:\projects\evergreen\evergreen\apps\api\src\auth\auth.service.ts` (what login/refresh/me actually return and their status codes).
4. The shared contract types: `D:\projects\evergreen\evergreen\packages\shared-types\src\auth.ts` and `.../common.ts`.
5. The UI primitives and design tokens: `apps/mobile/src/components/ui/*`, `apps/mobile/tailwind.config.js`.
6. The scaffold files being modified: `apps/mobile/src/lib/api.ts`, `apps/mobile/src/lib/keychain.ts` (pre-existing behavior).

Your job: check for violations of the spec's acceptance criteria, boundaries/constraints, and I/O matrix rows, and of the Story 1.6 ACs in epics.md. For each AC / matrix row / boundary, verify the diff satisfies it. Report:

- **PASS** — the AC/row/boundary is satisfied (name it, cite the line that satisfies it).
- **FAIL** — the AC/row/boundary is violated or not implemented (name it, cite the line, explain the gap).
- **UNVERIFIABLE** — the AC depends on manual/device behavior this diff can't prove (be specific about what the human must test).

Acceptance areas to audit explicitly:
1. Valid credentials → token pair in keychain, authenticated, splash resolves to role route.
2. Invalid credentials → inline error, no token issued.
3. 429 on login → human message, NO auto-retry.
4. 429 on refresh → screen intact, session NOT destroyed, deferred.
5. Network loss on login → inline connection error, inputs preserved.
6. Refresh on expired access token → transparent single retry.
7. Splash resolution: family → onboarding placeholder, other roles → home placeholder, no tokens → login.
8. Login greeting uses `{typography.hero}`.
9. Tokens only via keychain (never AsyncStorage / plain state).
10. `/auth/me` 401 + refresh fail → tokens cleared → login.
11. Boundaries: no logout UI/session-expiry screen (1.11), no role nav (1.10), no onboarding flow (1.8).
12. All types from `@evergreen/shared-types` (AD-2).
13. `saveTokens` rollback-safe (spec-16 deferred item).

Also flag any place where the spec and the diff disagree (bad_spec candidates) — but do NOT propose code; just state the disagreement and which side looks wrong.

For each finding: **Severity** (critical / major / minor / nit), **File**:line, **What**, **Which AC/constraint it touches**.
