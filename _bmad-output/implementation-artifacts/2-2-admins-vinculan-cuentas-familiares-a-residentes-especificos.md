...
- **Not verified in a browser:** Claude-in-Chrome is not connected in this environment (confirmed via `tabs_context_mcp`), so the new "Family links" dialog was verified by type-check + lint + code review against the established `role-users-panel.tsx` pattern only, not by driving the actual UI. Backend behavior for every new endpoint is covered by the e2e suite (real HTTP requests against a real Postgres instance). Recommend a manual click-through before merge.
- Started local Postgres (`docker compose up -d`, Docker Desktop was not running at session start) to run the e2e suite — left running.
