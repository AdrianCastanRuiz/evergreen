...
- **Post-ship deviation from AC #2 (2026-09-11, product decision)**: the pill-row (2–3) / dropdown (4+) split was replaced with a single always-dropdown switcher for any 2+ linked residents, at the requester's explicit direction ("cambia la UI a tu gusto si es necesario para que quede bonito") after a real-device screenshot showed the pill row overflowing the screen with long resident names. The dropdown now also shows each resident's photo/initials avatar and a checkmark on the active row. `resident-switcher.tsx`'s pill-row branch was removed entirely.
