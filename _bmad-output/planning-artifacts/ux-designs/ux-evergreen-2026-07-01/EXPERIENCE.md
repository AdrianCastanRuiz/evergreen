...
Family accounts can link to **multiple residents within the same care home** (e.g. two parents living in the same home). When 2+ residents are linked, a persistent **resident switcher** (`{components.resident-switcher}`) appears at the top of Home, Photos, Events, and Menu — every one of those screens is implicitly scoped to whichever resident is currently selected. Single-resident families never see the switcher at all. **[DECISION 2026-07-09]** Cross-home linking (a family account spanning two different care homes) is explicitly out of scope for V1 — `User.home_id` is a single field and the tenant-isolation model (RLS, guards) resolves one home per session; a family with residents in two different homes uses two separate accounts. Revisit only if a real cross-home family case emerges post-V1.
