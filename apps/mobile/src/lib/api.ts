...
export async function request<T>(
  ...
  // Empty body regardless of status code — 204 (auth/logout,
  // auth/password-reset) is the common case, but a route can also send an
  // empty body on 201 (e.g. POST /residents/:id/family-links), which
  // response.json() throws a SyntaxError on ("Something went wrong" surfaced
  // to the user for what was actually a successful request). Read as text
  // first so any empty body is handled the same way no matter the status.
